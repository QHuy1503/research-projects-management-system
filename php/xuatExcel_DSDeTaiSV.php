<?php
    ...
    $sql = "SELECT * FROM detai WHERE YEAR(NgayThucHien) = ?";
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            while($row_data = $result->fetch_assoc()) {
                $sheet->setCellValue('A' . $row, $row_data['MaDeTai']);
                $sheet->setCellValue('B' . $row, $row_data['MaNhom']);
                ...
                $sheet->setCellValue('G' . $row, $row_data['GVCoVan']);
            ...
            $writer->save('C:\Users\USER\Desktop\DanhSachDeTaiSV'.$nam.'.xlsx');
            echo json_encode(array('success' => true, 'message' => 'File Excel đã được tạo thành công!'));
        } else {
            echo json_encode(array('success' => false, 'message' => 'Lỗi: ' . $stmt->error));
        }
    }
?>